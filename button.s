...
GPIO_PORTN_DIR_R              EQU     0x40064400         ;GPIO Port N DIR Register
GPIO_PORTN_DEN_R              EQU     0x4006451C         ;GPIO Port N DEN Register
GPIO_PORTN_DATA_R             EQU     0x400643FC         ;GPIO Port N DATA Register 
...
PortNM_Init 
		;STEP 1
		 ...
		 ORR R0,R0, #0x1800					; Perform a bitwise OR operation with contents of R0 and 0x1800 and stores it in R0
		 STR R0, [R1]						; Store what is in R0 into R1
		 NOP
		 NOP
		
		;STEP 5
		 LDR R1, =GPIO_PORTN_DIR_R			; Load the memory address of the GPIO Port N DIR Register into R1
		...
		;STEP 7
		 LDR R1, =GPIO_PORTN_DEN_R			; Load the memory address of the GPIO Port N DEN Register into R1
...
Start 
	    BL PortNM_Init       ; calls and execute your PortN_Init function
		;STEP 8    
		
		 LDR R1, =GPIO_PORTN_DATA_R			; Load the memory address of the GPIO Port N DATA Register into R1
		 LDR R0, [R1]						; Load the contents of the memory address of GPIO Port N DATA Register in R0

blink	 LDR R2, =GPIO_PORTM_DATA_R			; Load the memory address of the GPIO Port N DATA Register into R2
